...
<div id="page" class="site home-page">
	...
	<header id="masthead" class="site-header">
		<?php if( $newscard_settings[ 'newscard_top_bar_hide' ] == 0 ) { ?>
			<div class="info-bar<?= has_nav_menu( 'right-section' ) ? ' infobar-links-on' : '' ?>">
				<div class="container">
					<div class="row gutter-10">
						<div class="col col-sm contact-section">
							<div class="date">
								<ul><li><?= esc_html( date_i18n( "l, F j, Y" ) ) ?></li></ul>
							</div>
						</div><!-- .contact-section -->

						<?php if( $newscard_settings[ 'newscard_social_profiles' ] != '' && $newscard_settings[ 'newscard_top_bar_social_profiles' ] === 0 ) { ?>
							<div class="col-auto social-profiles order-md-3">
								<?= esc_html( newscard_social_profiles() ) ?>
							</div><!-- .social-profile -->
						<?php }

						if( has_nav_menu( 'right-section' ) ) { ?>
							<div class="col-md-auto infobar-links order-md-2">
								<button class="infobar-links-menu-toggle"><?= esc_html_e( 'Responsive Menu', 'newscard' ) ?></button>
								<?php 
									wp_nav_menu( array(
				...
				<div class="container">
					<div class="row navbar-head-row align-items-center">
						...
						<?php if ( $newscard_settings['newscard_header_add_image'] !== '' ) { ?>
							<div class="col-12 text-center navbar-ad-section">
				...
				<div class="navbar-main">
					<div class="container">
						<div class="collapse navbar-collapse" id="navbarCollapse">
							<div id="site-navigation" class="d-flex main-navigation<?php echo ($newscard_settings['newscard_nav_uppercase'] == 1) ? " nav-uppercase" : "";?>" role="navigation">
								<?php
					...
					<?php 
						if( 
							( 
								...
								( 
									is_home() && 
									$newscard_settings[ 'newscard_banner_display' ] === 'front-blog' 
								) 
							) && 
							( 
								$newscard_settings[ 'newscard_banner_slider_posts_hide' ] === 0 || 
								$newscard_settings[ 'newscard_banner_featured_posts_1_hide' ] === 0 ||
								$newscard_settings[ 'newscard_banner_featured_posts_2_hide' ] === 0 
							) 
						) {
							...
							if( 
								...
								(
									$newscard_settings[ 'newscard_banner_featured_posts_1_hide' ] === 1 && 
									$newscard_settings[ 'newscard_banner_featured_posts_2_hide' ] === 0
								) || 
								( 
									$newscard_settings[ 'newscard_banner_featured_posts_1_hide' ] === 0 && 
									$newscard_settings[ 'newscard_banner_featured_posts_2_hide' ] === 1
								) 
							) {
						...
						?>
						<div class="row gutter-parent-10 <?= $col_wrap_class ?>">
							<?php 
								if( $newscard_settings[ 'newscard_banner_slider_posts_hide' ] == 0 ) {
								...
								<div class="<?= $col_slider_class ?>">
									<div class="featured-slider post-slider<?= $newscard_settings['newscard_banner_slider_posts_title'] === '' ? " slider-no-title" : "" ?>">
										<div class="owl-carousel">
											<?php 
												while( $newscard_get_banner_slider->have_posts() ) {
							...
							<?php 
								if( $newscard_settings[ 'newscard_banner_featured_posts_1_hide' ] == 0 ) {
									$newscard_fp_cat 	= absint( $newscard_settings[ 'newscard_banner_featured_posts_1_post_categories' ] );
									$post_type_fp 		= array(
										'posts_per_page' 	=> 4,
										'post__not_in' 		=> get_option('sticky_posts'),
										'post_type' 			=> array( 'noticias' )
									...
									if( $newscard_settings[ 'newscard_banner_featured_posts_1_latest_post' ] == 'category' ) {
										$post_type_fp[ 'category__in' ] = $newscard_fp_cat;
									}

									$newscard_get_featured_post = new WP_Query( $post_type_fp ); 
							?>
								...
								<div class="<?php echo $col_mid_class ?>">
									<div class="featured-post">
										<div class="row">
											<?php 
												for($i = 0; $i < 2; $i++ ) {
													if( $newscard_get_featured_post->have_posts() ) {
														$newscard_get_featured_post->the_post();
											?>
							...
							<?php 
								...
								if( $newscard_settings[ 'newscard_banner_featured_posts_2_hide' ] == 0 ) {
							?>
								<div class="<?= $col_mid_class ?>">
									<div class="featured-post">
										<div class="row">
											<?php 
												while($newscard_get_featured_post->have_posts()) {
													$newscard_get_featured_post->the_post(); 
											?>
													<div class="<?php echo $col_mid_child_class ?>">
														<div class="post-item post-block">
